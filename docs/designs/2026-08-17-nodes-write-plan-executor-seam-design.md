...
| 2026-09-12 | §3 | Beliefs' answer to the 2026-09-11 §3 row, from its gate against merged 2.0: the symlink preflight and the segment/portability/reserved rules are accepted; the `.md` suffix rule is **refused** — Beliefs' durable executor takes `validate_plan` as its one lexical authority and writes its manifests, registry and epoch records through the seam. The suffix rule is withdrawn from `validate_plan` and STANDARD §4.1 the same day. | maintainer, on Beliefs' evidence | Beliefs: recorded — refusal of the suffix clause, acceptance of the rest |
