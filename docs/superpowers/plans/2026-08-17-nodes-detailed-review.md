...
- **Supply surface** (spec §3 item 2's last bullet): an executor is **root-bound at construction** — `DefaultExecutor(root)`; the protocol itself stays `execute(plan)` with no root parameter, so plans stay root-relative values. Injection is by **root-taking factory**, so a mismatch between the corpus root and the executor's root is unconstructible: `Corpus(root, executor_factory=...)` calls the factory with its own root (`executor_factory(root) -> WritePlanExecutor`; TypeScript `(root: string) => WritePlanExecutor`); omitted → the corpus constructs `DefaultExecutor(root)` itself. A composition root closes over its engine handles and lets the corpus supply the root — it never hands `Corpus` a pre-bound executor whose root the corpus cannot verify. The seam's public error additions are exactly `PlanRefusedError` and `ExecutionError`. These are the pieces science's adapter wires without re-opening `nodes`.
...
- Read (evidence): science `docs/designs/2026-08-17-conformance-cut-4.md` §1–§2 (the add-only slice, the chained-but-unanchored close), science `docs/designs/2026-08-03-redesign-adoption-ledger.md` rows 3–4, spec §3 item 5; **the engine itself**: `~/d/atoms/python/src/atoms/core/effects.py` (`CreateFileNoClobber(effect_id, path, post)`; `ReplaceFile(effect_id, path, pre, post)`; `DeletePath(effect_id, path, pre)`; `MoveNoClobber`), `~/d/atoms/python/src/atoms/core/fingerprint.py:22` (`FileState(content_hash, mode, byte_len)` — `content_hash` is `sha256:<64 hex>`-prefixed), `~/d/atoms/python/src/atoms/core/spec.py:31-40` (`TransactionSpec`: `schema_version`, `consumer_tag`, `intent_digest`, `initial_surface`, `final_surface`, `effects`, `dependencies`, `fulfills`, `registered_paths`), and `~/d/atoms/python/src/atoms/core/compiler.py:207-213` (`intent_digest` is caller-supplied; the engine validates only its `sha256:<64 lowercase hex>` format)
...
- **Digest formats differ and the conversion is stated:** the plan's `expected_digest` is raw lowercase hex; `FileState.content_hash` is `sha256:`-prefixed (`fingerprint.py`) — the adapter prefixes when building states and strips when cross-checking.
- `TransactionSpec` fields: `schema_version` (engine constant), `consumer_tag` and `effect_id`s (adapter-minted), `intent_digest` (**caller-supplied**: the adapter digests its own canonical intent encoding — derivable from the plan alone — and the engine validates only the `sha256:<64 lowercase hex>` format, compiler.py:212-213), `initial_surface`/`final_surface` (derived from the pre/post states above), `dependencies`/`fulfills` (adapter's call; none required by the plan), `registered_paths` — **pinned `()` for this slice**: the corpus-write adapter reserves nothing (cut 4's boundary), and this field is distinct from the automatic registration-chain append, which the engine performs in every transaction regardless, invisible to `nodes` (science cut 4's "chained but unanchored" fact — record it verbatim).
...
Close with what the adapter supplies from its side: the root-taking executor factory handed to `Corpus`, closing over the engine handles, and the composition root's exclusive ownership of executor choice.
...
The rule, applied plainly: if **any** surviving withdrawal removes a surface a §11 fixture pins, the amendment is **2.0 (major)** — deleting pinned behavior with its fixture changes pinned tier-2 behavior under §12's text, and no alternative reading is available. **1.3 (minor)** is reachable only when every surviving withdrawal leaves the pinned surfaces untouched. Record whichever ruling the Task 5–6 verdicts imply, with the pinned-surface inventory in the annotation. (If the review believes §12's boundary itself is defective, that is a standard bug to raise for the implementation plan's amendment commit, not a reading this ruling may adopt.)
